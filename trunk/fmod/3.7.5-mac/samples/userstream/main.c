...
*/
signed char streamcallback(FSOUND_STREAM *stream, void *buff, int len, void *param) 
{
	int				count;
...
int main()
{
	...
	switch (key)
	{
	    case '1' :  FSOUND_SetOutput(FSOUND_OUTPUT_MAC);
	                break;
            case '2' :	FSOUND_SetOutput(FSOUND_OUTPUT_NOSOUND);
                        break;
            default : exit(0);
	}
