...
namespace
{
	class Logic : public SelectionLogic
	{
	public:
		Logic(wxInt32 index, boost::weak_ptr<Game> game) 
			: SelectionLogic(game) 
			, mIndex(index)
		{
			wxASSERT(-1 != index);
		...
		virtual void Initialize()
		{
			...
			// They lose half, rounded down.
			mLoss = total / 2;

			// The most resources they can select is the min of their amount
			// to lose or the resources available.
			wxInt32 maxImages = std::min(mLoss, size);
			selection->SetMaxImages(maxImages);

            static const wxString stSelectXResourcesToLose = _("Select %d resources to lose.");
	
			wxBaseStaticText* text = GetText();
			wxString str = wxString::Format(stSelectXResourcesToLose, 
				mLoss);
			text->SetLabel(str);
		}
...
//---------------------------- PUBLIC           -----------------------------//
wxLoseCardsDialog::wxLoseCardsDialog(wxWindow *parent, wxInt32 index, 
	boost::weak_ptr<Game> game, const HashString& rule)
: wxResourceSelectionDialog(parent, game, 
	SelectionLogicPtr(new Logic(index, game)), rule, _("Cards To Lose"), index)
{
}
