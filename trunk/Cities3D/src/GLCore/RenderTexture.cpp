...
#elif __MACH__
    if (err = CGLSetCurrentContext(_hPreviousContext)) return false;
#else
...
#ifdef _WIN32
    _vsnprintf_s( strBuffer, 256, strMode, args );
#else
...
{
    if (!modeString || strcmp(modeString, shEmpty.c_str()) == 0)
        return;
    ...
    char *buf = strtok(mode, shSpace.c_str());
    while (buf != NULL)
    {
        tokens.push_back(buf);
        buf = strtok(NULL, shSpace.c_str());
    }
    ...
    for (unsigned int i = 0; i < tokens.size(); i++)
    {
        ...
        if (kv.first == shRgb && (_iNumComponents <= 1))
        {           
            if (kv.second.find(shF) != kv.second.npos)
                _bFloat = true;
        ...
        if (kv.first == shRgba && (_iNumComponents == 0))
        {
            if (kv.second.find(shF) != kv.second.npos)
                _bFloat = true;
		...
		else if (kv.first == shRgba) 
            fprintf(stderr, 
                    "RenderTexture Warning: mistake in components definition "
                    "(rgba + %d).\n", 
                    _iNumComponents);
        
        if (kv.first == shR && (_iNumComponents <= 1))
        {
            if (kv.second.find(shF) != kv.second.npos)
                _bFloat = true;
		...
		else if (kv.first == shR) 
            fprintf(stderr, 
                    "RenderTexture Warning: mistake in components definition "
                    "(r + %d).\n", 
                    _iNumComponents);

        if (kv.first == shRg && (_iNumComponents <= 1))
        {
            if (kv.second.find(shF) != kv.second.npos)
                _bFloat = true;
        ...
        if (kv.first == shDoubleBuffer || kv.first == shDouble)
        {
#ifdef _WIN32
        ...
        if (token.find(shTex) == 0)
        {            
            _bIsTexture = true;
            
            if ((kv.first == shTexRECT) && GLEW_NV_texture_rectangle)
            {
                _bRectangle = true;
                bBindRECT = true;
            }
            else if (kv.first == shTexCUBE)
            {
                bBindCUBE = true;
            }
            else
        ...
        if (token.find(shDepthTex) == 0)
        {
            _bIsDepthTexture = true;
            
            if ((kv.first == shDepthTexRECT) && GLEW_NV_texture_rectangle)
            {
                _bRectangle = true;
                bBindRECT = true;
            }
            else if (kv.first == shDepthTexCUBE)
            {
                bBindCUBE = true;
            }
            else
...
RenderTexture::KeyVal RenderTexture::_GetKeyValuePair(string token)
{
    ...
    if ((pos = token.find(shEquals)) != token.npos)
    {
...
vector<int> RenderTexture::_ParseBitVector(string bitVector)
{
    ...
    do
    { 
        nextpos = bitVector.find_first_of(shCommaSpaceNewline, pos);
...
bool RenderTexture::_InitializeTextures()
{
    ...
    if (_bIsTexture)
    {
        ...
        {
            GLuint iInternalFormat = GL_RGBA;
            GLuint iFormat;
...
#elif __MACH__
    CGLError err;
    if (err = CGLSetCurrentContext(_hGLContext)) return false;
#else
    if (false == glXMakeCurrent(_pDisplay, _hPBuffer, _hGLContext)) 
    {
        return false;
...
{   
    ...
    string mode = shEmpty;
    if (bDepth)
        mode.append(shDepthSpace);
    if (bStencil)
        mode.append(shStencilSpace);
    if (bMipmap)
        mode.append(shMipmapSpace);
    if (iRBits + iGBits + iBBits + iABits > 0)
    {
        ...
        if (iABits > 0)
            mode.append(shA);
        mode.append(shEquals);
        char bitVector[100];

        sprintf(bitVector,
            shBitFormat.c_str(),
            iRBits, (iRBits >= 16) ? shF : shEmpty,
            iGBits, (iGBits >= 16) ? shF : shEmpty,
            iBBits, (iBBits >= 16) ? shF : shEmpty,
            iABits, (iABits >= 16) ? shF : shEmpty);
    ...
    if (_bIsTexture)
    {
        if (GLEW_NV_texture_rectangle && 
            ...
            mode.append(shTexRECTSpace);
        else
            mode.append(shTex2DSpace);
    }
    if (_bIsDepthTexture)
    {
        if (GLEW_NV_texture_rectangle && 
