...
//---------------------------------------------------------------------------//
// Class: RulePlaceDeserter
// 
// Places a deserter on the board during a player's turn.
//
// Derived From:
//     <Rule>
//
...
//     <LogicCostKnight>
//
class RulePlaceDeserter : public Rule
{
public:
	virtual void Execute(const DataObject &object)
	{
		...
		wxInt32 strength = gameData<wxInt32>(shDeserterKnightStrength);
		wxInt32 upgrade = gameData<wxInt32>(shDeserterKnightUpgrade);
		wxInt32 active = gameData<wxInt32>(shDeserterKnightActive);
		...
		wxASSERT(0 <= playerGameData<wxInt32>(type));
		Controller::get().Transmit(shEventPlayerUI, 
			DataObject(GetGame(), current()));

		// Play the sound.
		RULE.Execute(shRulePlaySound, DataObject(SOUND_PLACE_DESERTER_KNIGHT));
		
		Controller::get().Transmit(shEventKnightCounts, GetGame());

		// Update the UI.
		RULE.Execute(shRuleUpdateNetworkUI, DataObject(stPlacesTheDeserter));
...
IMPLEMENT_RULE(RulePlaceDeserter, CITIES)
