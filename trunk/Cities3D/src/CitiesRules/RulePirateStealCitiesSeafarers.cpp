...
//---------------------------------------------------------------------------//
// Class: RulePirateStealCitiesSeafarers
// 
// Looks for other ships on the given tile to steal a resource from, unless 
// we're in Bishop mode, in which case all players on the tile get stolen 
// from.
//
// Derived From:
//     <Rule>
...
//     <Seafarers>
//
class RulePirateStealCitiesSeafarers : public Rule
{
public:
	virtual void Execute(const DataObject &object)
	{
		DataObject output;
		RULE.Decide(shLogicPirateStealPlayers, object, output);
		...
		if(1 <= players.size())
		{
			if(false == bishop)
			{
				gameData<HashString>(shState) = shPirateSteal;
				RULE.Execute(shRuleRestartPirateSteal, object);
			}
			else
			{
...
IMPLEMENT_RULE_REPLACE(RulePirateStealCitiesSeafarers, RulePirateSteal,
					   CITIES_SEAFARERS)
