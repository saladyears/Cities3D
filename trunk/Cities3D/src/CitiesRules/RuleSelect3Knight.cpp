...
public:
	virtual void Execute(const DataObject &object)
	{
		...
		// Update the UI.
		RULE.Execute(shRuleUpdateNetworkUI, DataObject(stUpgradesToAMightyKnight));
		...
		// They lose one level 3 knight and gain a level 2 knight.
		playerGameData<wxInt32>(shStockKnights2) += 1;
		playerGameData<wxInt32>(shStockKnights3) -= 1;

		// Play the upgrade sound.
		RULE.Execute(shRulePlaySound, DataObject(SOUND_UPGRADE_KNIGHT_3));
		...
		// Adjust stats.
		RULE.Execute(shRulePurchaseItem, DataObject(current(), shKnight3));
	...
	virtual void Unexecute(const DataObject &object)
	{
		...
		// They gain one level 3 knight and lose a level 2 knight.
		playerGameData<wxInt32>(shStockKnights3) += 1;
		playerGameData<wxInt32>(shStockKnights2) -= 1;
...
IMPLEMENT_RULE(RuleSelect3Knight, CITIES)
