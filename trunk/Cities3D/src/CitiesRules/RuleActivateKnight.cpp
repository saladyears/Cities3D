...
//---------------------------------------------------------------------------//
// Class: RuleActivateKnight
// 
// Mixin for handling a click on the activate knight <BuildUI> in the game
// state of Turn, allowing the player to activate a knight on the board.
//
// Derived From:
//     <Rule>
//
...
//
// Logic Called:
//     <LogicCanPlaceActivateKnight>
//
// Selection Rule:
//     <RuleSelectActivateKnight>
//
// Game State:
//     TurnActivateKnightPlacement
//
// Transmits To:
//     eventBuildUI
...
public:
	virtual void Execute(const DataObject &)
	{
		...
		RULE.Execute(shRuleExecuteAutoTrade, DataObject(
			shRuleActivateKnight, shLogicCostActivateKnight));
		...
		// Update the UI text.
		RULE.Execute(shRuleUpdateMessageUI, DataObject(stActivateAKnight, 
            stWaitingActivateAKnight));

		// This is a new game state.
		gameData<HashString>(shState) = shTurnActivateKnight;
		...
		Controller::get().Transmit(shEventSelectionObject, 
			SelectionObjectPtr(
			new DummySelectionObject(shRuleSelectActivateKnight)));
...
IMPLEMENT_RULE(RuleActivateKnight, CITIES)
