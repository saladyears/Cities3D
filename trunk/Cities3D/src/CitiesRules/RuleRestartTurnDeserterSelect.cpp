...
//---------------------------------------------------------------------------//
// Class: RuleRestartTurnDeserterSelect
// 
// Restarts the game from a player choosing which of their knights will desert.
//
// Derived From:
//     <Rule>
//
...
//     <RuleRestartGame>
//
class RuleRestartTurnDeserterSelect : public Rule
{
public:
	virtual void Execute(const DataObject &)
	{
		...
		RULE.Execute(shRuleUpdateMessageUI, DataObject(stSelectAKnightToDesert, 
            stWaitingSelectAKnightToDesert));

		Controller::get().Transmit(shEventControlsUI, DataObject(false));
		Controller::get().Transmit(shEventBuildUI, 
				DataObject(false, GetGame()));

		// Turn on all of their knights as potential selections.
		...
		Controller::get().Transmit(shEventSelectionObject, 
			SelectionObjectPtr(
			new DummySelectionObject(shRuleSelectDeserterKnight)));
...
IMPLEMENT_RULE_KEYED_MIXIN(RuleRestartTurnDeserterSelect, TurnDeserterSelect, 
