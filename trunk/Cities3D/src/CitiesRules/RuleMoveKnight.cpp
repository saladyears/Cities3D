...
// 
// Mixin for handling a click on the move knight <BuildUI> in the game
// state of Turn, allowing the player to move a knight on the board.
//
// Derived From:
//     <Rule>
//
...
//
class RuleMoveKnight : public Rule
{
public:
	virtual void Execute(const DataObject &)
	{
		...
		RULE.Decide(shLogicCanMoveKnight, input, output);
		...
		// Update the UI text.
		RULE.Execute(shRuleUpdateMessageUI, DataObject(stSelectAKnightToMove, 
            stWaitingSelectAKnightToMove));

		// This is a new game state.
		gameData<HashString>(shState) = shTurnSelectMoveKnight;
		...
		Controller::get().Transmit(shEventSelectionObject, 
			SelectionObjectPtr(
			new DummySelectionObject(shRuleSelectMoveKnight)));
...
IMPLEMENT_RULE(RuleMoveKnight, CITIES)
