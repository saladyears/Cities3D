...
//---------------------------------------------------------------------------//
// Class: RuleRestartTurnMoveAttackedKnight
// 
// Restarts the game from a player selecting a location to move their knight
// that was attacked during their turn.
//
// Derived From:
//     <Rule>
//
...
//
class RuleRestartTurnMoveAttackedKnight : public Rule
{
public:
	virtual void Execute(const DataObject &)
	{
		...
		HashString rule = shRulePlaceAttackedKnight;
		RULE.Execute(shRuleBuildKnightCommon, DataObject(corners, rule));
		...
		RULE.Execute(shRuleUpdateMessageUI, 
			DataObject(stWithdrawYourKnight, stWaitingWithdrawAKnight));
	}
...
IMPLEMENT_RULE_KEYED_MIXIN(RuleRestartTurnMoveAttackedKnight, 
						   TurnMoveAttackedKnight, RuleRestartGame, CITIES)
