...
namespace
{
		...
		wxCardListCtrl::CardArray& cards)
	{
		...
		// Add all playable cards as enabled.
		...
		for(it = playableCards.begin(); it != itEnd; ++it)
		{
			...
			for(wxInt32 i = 0; i < count; ++i)
			{
                ...
                cards.push_back(
                    ...
                    playable ? color : wxColour(128, 128, 128), playable));
			}
		}
	}
...
//---------------------------- PUBLIC           -----------------------------//
wxProgressCardListCtrl::wxProgressCardListCtrl(wxWindow* parent, wxWindowID id, 
