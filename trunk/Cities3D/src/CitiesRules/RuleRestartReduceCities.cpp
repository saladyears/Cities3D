...
//---------------------------------------------------------------------------//
// Class: RuleRestartReduceCities
// 
// Restarts the game from players losing cities to the barbarians
//
// Derived From:
//     <Rule>
//
...
//     <RuleRestartGame>
//
class RuleRestartReduceCities : public Rule
{
public:
	virtual void Execute(const DataObject &)
	{
		...
		{
			if(TRUE == playerGameData<wxInt32>(shLoseCity, i))
			{
				...
				// See if this player is losing a city.
				...
				if(true == output.read<bool>())
				{
					...
					// Turn on all destroyable cities for this player.
					DummySelectionObject* selection = new DummySelectionObject(
						shRuleSelectReduceCity);
		...
		if(true == done)
		{
			...
			RULE.Execute(shRuleResetBarbarians, DataObject());
		}
		else
		{
			wxString str1;
			...
			if(true == thisPlayer)
			{
				str1 = wxString::Format(stSelectACityToReduce.c_str(),
					thisName.c_str());
			}
			else
			{
				str1 = wxString::Format(stWaitingReduceACity.c_str(),
					str.c_str());
			}
...
IMPLEMENT_RULE_KEYED_MIXIN(RuleRestartReduceCities, ReduceCities, 
