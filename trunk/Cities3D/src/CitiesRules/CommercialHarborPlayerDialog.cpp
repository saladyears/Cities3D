...
namespace
{
	wxImage CreateImageCommercialHarborPlayer(ColorType color, wxInt32 total)
	{
	...
	void PopulateListCommercialHarborPlayer(wxBaseListCtrl* list,
		boost::weak_ptr<Game> thisGame)
	{
		const GamePtr& game = thisGame.lock();
		wxASSERT(game);

		// Only add players that have commodities.
		...
		{
			if( (i != current) &&
				(TRUE == game->playerGame(i).read<wxInt32>(shCommercialHarbor)))
			{
				const Data::IntHash& resources = 
					game->playerGame(i).read<Data::IntHash>(shResources);
				wxInt32 commodityTotal = 0;
				wxInt32 total = 0;
				...
				if(0 < commodityTotal)
				{
					...
					wxImage image = CreateImageCommercialHarborPlayer(color, total);

					const wxString& name = game->playerGame(i).player().Name();
					wxString str = wxString::Format(swStringParenInt.c_str(), 
                        name.c_str(), game->playerGame(i).read<wxInt32>(shPoints));
...
//---------------------------- PUBLIC           -----------------------------//
wxCommercialHarborPlayerDialog::wxCommercialHarborPlayerDialog(wxWindow *parent, 
	boost::weak_ptr<Game> game)
: wxPlayerSelectionDialog(parent, game, shRuleSelectCommercialHarborPlayer, 
	stSelectAPlayerToTradeWith, boost::bind(&PopulateListCommercialHarborPlayer, _1, _2))
{
}
