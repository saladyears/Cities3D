...
//---------------------------------------------------------------------------//
// Class: NetworkRuleSpectatorState
// 
// Sets the replacement state of the given spectator.
//
// Derived From:
//     <Rule>
//
...
//     <Network>
//
class NetworkRuleSpectatorState : public Rule
{
public:
	virtual void Execute(const DataObject &object)
	{
		...
		const wxBOOL state = object.read<wxBOOL>(1);
		...
		for(it = spectators.begin(); it != itEnd; ++it)
		{
				...
				(it->second != state))
			{
				...
				// Check to see if there's any spots free.
...
IMPLEMENT_RULE(NetworkRuleSpectatorState, NETWORK)
