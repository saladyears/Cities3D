...
//---------------------------------------------------------------------------//
// Class: LogicCanPlaceSettlement
// 
// Determines the available board locations for a settlement to be placed for 
// the current player.
//
// Derived From:
//     <Logic>
...
//
class LogicCanPlaceSettlement : public Logic
{
public:
	virtual void Decide(const DataObject &, DataObject &output)
	{
		...
		//    settlement can be built there.  If not, remove the location from
		//    the set.
		//

		TileCornerSet corners;

		// Step 1: Build the valid set of settlement locations.
		...
		while(it != itEnd)
		{
			...
			DataObject input(*itThis), output;
			RULE.Decide(shLogicIsValidSettlementPlacement, input, output);
...
IMPLEMENT_LOGIC(LogicCanPlaceSettlement, STANDARD)
