...
//---------------------------------------------------------------------------//
// Class: LogicCanPlaceShip
// 
// Determines the available board locations for a ship to be placed for the
// current player.
//
// Derived From:
//     <Logic>
...
//
class LogicCanPlaceShip : public Logic
{
public:
	virtual void Decide(const DataObject &, DataObject &output)
	{
		...
		// 3. For each potential location, run logic that determines if a ship
		//    can be built there.  If not, remove the location from the set.
		//

		TileSideSet sides;

		// Step 1: Build the valid set of ship locations.
		BuildValidSet(sides);
		...
		while(it != itEnd)
		{
			...
			RULE.Decide(shLogicIsValidShipPlacement, input, output);
			...
			if(FALSE == tile<wxInt32>(shBorder, i))
			{
				for(wxInt32 j = 0; j < 6; ++j)
				{
					...
					if(invalids.end() == invalids.find(side))
					{
						...
						// In addition, a ship may not be placed on the pirate
						// tile.
						if( (pirateTile == i) ||
							(pirateTile == otherTile))
						{
							insert = false;
						}
...
IMPLEMENT_LOGIC(LogicCanPlaceShip, SEAFARERS)
