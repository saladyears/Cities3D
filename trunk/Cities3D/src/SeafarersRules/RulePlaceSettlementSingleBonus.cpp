...
//---------------------------------------------------------------------------//
// Class: RulePlaceSettlementSingleBonus
// 
...
public:
	virtual void Execute(const DataObject &object)
	{
			...
			settlement)))
		{
			...
			// Update the UI.
			RULE.Execute(shRuleUpdateNetworkUI, DataObject(stReceivesOneForColonizing));
	...
	{
		// Only undo the points if they actually got an extra point on this
		// turn.
...
private:
		...
		CornerObjectPtr& object)
	{
		...
		// Now go through all the current settlements for the player and see
		// if any already exist on any of these tiles.
...
IMPLEMENT_RULE_MIXIN(RulePlaceSettlementSingleBonus, RulePlaceSettlementTurn, 
					 SINGLE_BONUS)
